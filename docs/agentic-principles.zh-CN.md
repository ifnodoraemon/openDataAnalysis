...
面向用户的界面、HTTP/SSE 状态文本、`ui_summary` 和运维命令行输出统一使用简体中文。面向模型的策略与工具契约统一使用英文。机器字段、枚举值、协议名、产品名和库名保留规范原文，不翻译也不创建别名。英文与中文文档分别使用 `.md` 和 `.zh-CN.md` 文件，不能在同一正文中混用两种叙述语言。
...
4. `history`：仍在窗口内的原始对话和工具结果。

窗口裁剪只在 `runtime` 层暴露省略消息数量，不由运行时编写语义摘要；需要跨窗口保留的判断由模型显式写入工作记忆。临时事实、历史状态和用户偏好不能提升到 `system` 层。子代理额外约束使用 `policy_appendix`，不能用它倾倒背景事实。
