...
const snapshotCols = `id, session_id, source_id, upstream_kind, upstream_schema, upstream_object, analysis_table_name, row_count, column_count, status, error_message, schema_signature, imported_at, rows_imported, rows_skipped, import_duration_ms, profile_duration_ms, snapshot_size_bytes, profile_mode`
...
func scanSnapshot(row interface{ Scan(...interface{}) error }) (*domain.SourceSnapshot, error) {
	...
	if err := row.Scan(&s.ID, &s.SessionID, &s.SourceID, &s.UpstreamKind, &s.UpstreamSchema, &s.UpstreamObject, &s.AnalysisTableName, &s.RowCount, &s.ColumnCount, &status, &errMsg, &s.SchemaSignature, &s.ImportedAt, &s.RowsImported, &s.RowsSkipped, &s.ImportDurationMs, &s.ProfileDurationMs, &s.SnapshotSizeBytes, &profileMode); err != nil {
		return nil, err
	}
...
func (r *SourceSnapshotRepository) UpdateRuntimeFacts(ctx context.Context, id string, rowsImported, rowsSkipped, importDurationMs, profileDurationMs int, snapshotSizeBytes int64, profileMode domain.ProfileMode) error {
	_, err := r.db.ExecContext(ctx,
		`UPDATE source_snapshots SET rows_imported = ?, rows_skipped = ?, import_duration_ms = ?, profile_duration_ms = ?, snapshot_size_bytes = ?, profile_mode = ? WHERE id = ?`,
		rowsImported, rowsSkipped, importDurationMs, profileDurationMs, snapshotSizeBytes, string(profileMode), id)
	return err
}

func (r *SourceSnapshotRepository) UpdateSnapshotCompletion(ctx context.Context, id string, rowCount, colCount int, schemaSignature string, rowsImported, rowsSkipped, importDurationMs, profileDurationMs int, snapshotSizeBytes int64, profileMode domain.ProfileMode) error {
	_, err := r.db.ExecContext(ctx,
		`UPDATE source_snapshots SET row_count = ?, column_count = ?, schema_signature = ?, rows_imported = ?, rows_skipped = ?, import_duration_ms = ?, profile_duration_ms = ?, snapshot_size_bytes = ?, profile_mode = ? WHERE id = ?`,
		rowCount, colCount, schemaSignature, rowsImported, rowsSkipped, importDurationMs, profileDurationMs, snapshotSizeBytes, string(profileMode), id)
	return err
}
